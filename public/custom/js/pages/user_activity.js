$(document).ready(function () {
    ...
    function fetchDashboardData() {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    const loginData = response.data.activities || [];
                    ...
                    if ($.fn.DataTable.isDataTable("#loginTable")) {
                        const table = $("#loginTable").DataTable();
                        table.clear().rows.add(loginData).draw();
                    } else {
                        $("#loginTable").DataTable({
                            ...
                            columns: [
                                ...
                                { data: "timestamp", title: "Login Time" },
                                { data: "ip_address", title: "IP Address" },
                                { data: "login_method", title: "Login Method" },
                                {
                                    data: "user_agent",
                                    title: "Device / Browser",
                                    render: function (data) {
                                        return `<div class="wrap-text">${
                                            data || "-"
                                        }</div>`;
                                    },
                                },
                            ...
                            columnDefs: [
                                {
                                    targets: 5, // Device / Browser column index
                                    width: "250px",
                    ...
                    $("#loginTable tbody").html(
                        '<tr><td colspan="8" class="text-center text-muted py-4">Error fetching data: ' +
    ...
    function updateDashboard(stats) {
        ...
        $("#peakHourDisplay").text(
            stats.peak_hour ? `${stats.peak_hour}:00` : "-"
        );
        ...
        $("#tokenPercentage").text(
            total ? ((stats.token_logins / total) * 100).toFixed(1) + "%" : "0%"
        );
        $("#normalPercentage").text(
            total
                ? ((stats.normal_logins / total) * 100).toFixed(1) + "%"
                : "0%"
        );

        // Update hourly chart stats
        const morningLogins = stats.hourly_counts
            ? Object.values(stats.hourly_counts)
                  .slice(6, 12)
                  .reduce((sum, val) => sum + val, 0)
            : 0;
        const afternoonLogins = stats.hourly_counts
            ? Object.values(stats.hourly_counts)
                  .slice(12, 18)
                  .reduce((sum, val) => sum + val, 0)
            : 0;
        const eveningLogins = stats.hourly_counts
            ? Object.values(stats.hourly_counts)
                  .slice(18, 24)
                  .reduce((sum, val) => sum + val, 0)
            : 0;
        ...
        const totalLogins = Object.values(dailyCounts).reduce(
            (sum, val) => sum + val,
            0
        );
        const highestDay = dates.length
            ? dates.reduce((a, b) => (dailyCounts[a] > dailyCounts[b] ? a : b))
            : "--";
        $("#highestDay").text(
            highestDay !== "--"
        ...
        $("#averagePerDay").text(
            dates.length ? (totalLogins / dates.length).toFixed(1) : 0
        );
    ...
    function updateTable() {
        ...
        filteredData.forEach((login, index) => {
            const employeeId =
                login.employee_id && login.employee_id !== "-"
                    ? login.employee_id
                    : "--";
            const row = `
                <tr>
    ...
    function updateUserRankingTable() {
        ...
        filteredData.forEach((login) => {
            ...
            userLastLogin[key] =
                login.timestamp &&
                (!userLastLogin[key] ||
                    new Date(login.timestamp) > new Date(userLastLogin[key]))
                    ? login.timestamp
                    : userLastLogin[key];
        ...
        sortedUsers.forEach(([user, count], index) => {
            const row = `
                <tr>
                    ...
                    <td>${
                        userLastLogin[user]
                            ? new Date(userLastLogin[user]).toLocaleString()
                            : "-"
                    }</td>
